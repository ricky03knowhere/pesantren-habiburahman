...
const FormPembayaran = () => {
  ...
  useEffect(async () => {
    ...
    const filteredData = getData.filter((el) => el.status === "false");
  ...
  const handleSubmit = async () => {
    ...
    if (postData.id === "null") {
      ...
      await axios
        ...
        .then(({ data }) => {
          console.log(data);
          // return (window.location.href = "/pembayaran");
        })
          ...
          <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
            <div class="bg-gradient-info shadow-info border-radius-lg pt-4 pb-3">
              <h5 class="text-white text-capitalize ps-3 font-weight-bold">
                <i class="material-icons opacity-10 me-2" style={{transform: 'scale(1.3)'}}>receipt_long</i>Form
                Pembayaran
              </h5>
            </div>
          </div>
